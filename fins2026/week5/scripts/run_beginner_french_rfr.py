# ruff: noqa
"""Download the Week 5 daily risk-free rate from Kenneth French."""
...
from fins2026.week5.code.risk_free_rate_french import (
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Parquet file for the Week 5 crypto merge."
        ),
    )
    parser.add_argument(
        "--url",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Week 5 daily risk-free rate download")
    print()
    print("What this shows:")
    print("- not every external source is a JSON API; some are direct file downloads")
    print(
        "- the crypto panel trades every day, but Kenneth French RF only "
        "arrives on business days"
    )
    print("- Week 5 keeps only the final clean RF dataset, not intermediate raw files")
