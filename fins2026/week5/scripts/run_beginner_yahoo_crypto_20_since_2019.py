...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Download the Week 5 Yahoo 20-coin panel and save it in long format.",
    )
    parser.add_argument(
        "--tickers-file",
        default=str(DEFAULT_YAHOO_CRYPTO_PANEL_FILE),
        help="Repo-relative or absolute 20-coin universe file.",
    )
    parser.add_argument(
        "--start-date",
        default="2019-01-01",
        help="First requested date for the long panel. Default: 2019-01-01.",
    )
    parser.add_argument(
        "--end-date",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(DEFAULT_YAHOO_CRYPTO_FULL_OUTPUT_DIR),
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Pause between ticker requests. Default is more conservative than the warm-up pull.",
    )
    parser.add_argument(
        "--timeout-seconds",
        type=int,
    ...
    parser.add_argument(
        ...
        default=True,
        help="Reuse per-ticker cache in the output directory. Default: on.",
    )
...
def main(argv: list[str] | None = None) -> int:
    """Pull the full Yahoo crypto panel used later in the week."""
    ...
    print("Week 5 Yahoo 20-coin crypto pull")
    print()
    print("What this shows:")
    print("- Yahoo is the canonical Week 5 crypto source")
    print("- resume mode and retry logic matter because Yahoo can be brittle")
    print("- the saved panel is long format and quoted in USD")
    print("- the coverage summary audits whether each coin reaches the requested 2019 start date")
    print()
    ...
    if "failures_csv" in outputs:
        ...
        raise SystemExit(
            "Yahoo pull completed with failed tickers. Re-run with --resume "
            "to retry the missing names."
        )
    return 0
