...
agent_edited = [
    ...
    not in (ROOT / f).read_text(encoding="utf-8", errors="ignore").lower()
]
check(
    bool(agent_edited),
    f"your own agent file ({', '.join(agent_edited) or 'none'})",
    "edit AGENTS.md or CLAUDE.md (your tool's file) with your own instructions - "
    "both are still the provided stub",
)
...
check(
    not placeholder_left,
    "no leftover zID placeholder",
...
check(
    not junk, "no stray editor-backup or OS-junk files", f"remove these before zipping: {junk[:3]}"
)
...
warn(
    report,
    "report present (report/report.pdf or .docx)",
    "no report/report.pdf yet - author it in Word and export to PDF",
)
results_files = [p for p in (ROOT / "results").rglob("*") if p.is_file() and p.name != ".gitkeep"]
warn(
    bool(results_files),
    f"results/ has {len(results_files)} output file(s)",
...
required_outputs = {
    "A": ["results/tables/dataset_inventory.csv", "results/tables/descriptive_stats_returns.csv"],
    "B": [
...
if part == "B":
    ...
    app_text = (
        (ROOT / "streamlit_app.py").read_text(encoding="utf-8", errors="ignore")
        if (ROOT / "streamlit_app.py").exists()
        else ""
    )
    warn(
        "nltk" not in app_text,
        "app does not import nltk",
        "streamlit_app.py references nltk - the deployed app must read precomputed "
        "results/, not run VADER (the free tier cannot)",
    )
