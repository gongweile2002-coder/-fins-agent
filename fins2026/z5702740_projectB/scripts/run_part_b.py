...
def _write_report(metrics: pd.DataFrame, fusion_table: pd.DataFrame, design: pd.DataFrame) -> None:
    ...
    story.append(
        Paragraph(
            "FINS5545 Project Part B: Systematic Multi-Asset Funds with News Sentiment",
            styles["Title"],
        )
    )
    p(
        "This report draft documents a reproducible walk-forward experiment over the "
        "2020-2023 course dataset. The investment universe combines 50 US equities "
        "and 10 cryptocurrencies, with equity-only and crypto-only funds included "
        "for benchmarking. All portfolio weights are estimated using only trailing "
        "observations available at each monthly rebalance."
    )
    h("1. Fund design and backtest controls")
    design_text = "; ".join(
        f"{r.fund}: first live date {r.first_live_date}, "
        f"lookback {r.lookback_days} days, annualisation {r.periods_per_year}"
        for r in design.itertuples()
    )
    p(
        "The methods are equal weight, minimum variance, maximum Sharpe and "
        "inverse-volatility risk parity. Long-only weights are capped at 20 "
        "percent to reduce single-name concentration. "
        + design_text
        + "."
    )
    p(
        "Crypto returns are computed on the 365-day crypto calendar first. "
        "Combined funds then left-merge those already-computed crypto returns "
        "onto the equity trading calendar, matching the course calendar instruction."
    )
    ...
    p(
        f"The highest Sharpe in this run is {best['fund']} at "
        f"{best['sharpe']:.2f}. This should be interpreted as an out-of-sample "
        "historical result, not a forecast. Differences across funds largely "
        "reflect the trade-off between volatility control and exposure to "
        "high-return crypto assets."
    )
    for image, caption in [
        ...
        (
            ...
            "Figure 3. Top average holdings through time for the combined minimum-variance fund.",
        ),
    ]:
    ...
    p(
        "Headlines are de-duplicated by ticker, date and title, then aligned to "
        "the same or next equity trading day. A transparent finance lexicon scores "
        "the assembled ticker-day text. The sector index is equal-weighted across "
        "stocks, smoothed over five trading days, and lagged by one trading day "
        "before use in any portfolio rule."
    )
    story.append(
        Image(str(FIGURES / "sector_sentiment_index.png"), width=16.5 * cm, height=8.2 * cm)
    )
    p("Figure 4. Lagged sector sentiment index for the six most variable sectors.")

    h("4. Fusion extension")
    p(
        "The fusion test starts from the equity maximum-Sharpe weight schedule "
        "and tilts sector weights toward sectors with stronger lagged sentiment. "
        "Because the signal is lagged and measured before portfolio returns are "
        "realised, the test is look-ahead safe."
    )
    ...
    p(
        "The Streamlit app reads only precomputed CSV files from results/. It "
        "supports fund comparison, fund fact sheets, a simple allocation blend, "
        "current holdings and sector sentiment analytics. This design keeps the "
        "deployed app lightweight and avoids running NLP or portfolio optimisation "
        "on Streamlit Community Cloud."
    )
    p(
        "Recommendations: first, review the cap and rebalance frequency as "
        "sensitivity checks; second, treat the lexicon sentiment as a noisy proxy "
        "and compare it with VADER or a transformer model if time permits; third, "
        "report both positive and negative fusion results honestly because a weak "
        "signal is still informative for allocation governance."
    )
    ...
    md.write_text(
        ...
        "replace the placeholder zID folder name, and re-export from Word if "
        "required by the course.\n\n"
        + "\n".join(
            "- " + " ".join(wrap(line, 110))
            for line in [
                "The backtest is monthly, long-only and walk-forward with weights "
                "estimated from trailing data only.",
                "The sentiment model uses de-duplicated headlines aligned to the "
                "next trading day, smoothed five days and lagged one day.",
                "The deployed app reads precomputed CSV artifacts and does not "
                "import NLTK or recompute backtests.",
            ]
        ),
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    results.append(
        {"returns": tilted_returns, "weights": tilted_weights, "metrics": tilted_metrics}
    )
