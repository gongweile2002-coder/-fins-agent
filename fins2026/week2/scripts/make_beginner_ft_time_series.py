# ruff: noqa
"""Build the FT-style Week 2 beginner time-series chart."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Build the FT-style Week 2 beginner time-series chart.",
    )
    parser.add_argument(
        "--data",
...
def main(argv: list[str] | None = None) -> int:
    """Load the synthetic CSV and export the FT-style beginner line chart."""
    ...
    paths = export_ft_time_series(frame, args.output, repo_root=REPO_ROOT)
