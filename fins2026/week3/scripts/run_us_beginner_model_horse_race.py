# ruff: noqa
"""Run the Week 3 U.S. beginner forecast horse race."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run the Week 3 U.S. beginner forecast horse race.",
    )
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        ...
        help=(
            "Months used to choose the U.S. ARX exogenous variants inside the training sample."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    """Compare the U.S. extension models on one target-space horse-race table."""
    ...
    figure_paths = save_figure_pair(
        build_target_metric_race_figure(
            ...
            title="U.S. extension horse race: target RMSE across forecast families",
        ),
        args.figures_dir,
        "week3_us_beginner_horse_race",
        repo_root=REPO_ROOT,
    )

    best_row = metrics.sort_values(["target_rmse", "model"]).iloc[0]
    print("Week 3 U.S. beginner model horse race")
    print()
    ...
    print(
        "- ARX first chooses between the fedfunds-only and fedfunds-plus-spread variants "
