# ruff: noqa
"""Create the Week 3 beginner forecasting source CSVs."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Create the Week 3 beginner forecasting source CSVs.",
    )
    parser.add_argument(
        "--output-dir",
...
def main(argv: list[str] | None = None) -> int:
    """Write the real and simulated CSVs used by the Week 3 lecture ladder."""

    args = parse_args(argv)
    output_dir = resolve_repo_path(args.output_dir, REPO_ROOT)
    written = write_beginner_source_tables(output_dir, repo_root=REPO_ROOT)

    # Print the real-series sample so students can see the monthly lecture window.
    unemployment = load_fixture_unemployment_rate()
    print(f"real series sample: {sample_label(unemployment)}")

    # Print the simulated-sample range so students know those examples are deterministic.
