# ruff: noqa
"""Run the Week 3 beginner ARMA-forecast walkthrough."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run the Week 3 beginner ARMA-forecast walkthrough.",
    )
    parser.add_argument(
        "--data-dir",
...
def main(argv: list[str] | None = None) -> int:
    """Show the step from AR to ARMA on unemployment-rate changes."""
    ...
    # Step 2: load the level series and form the monthly-change target.
    ...
    # Step 3: select one AR lag and one ARMA order on the initial training sample.
    ...
    # Step 4: compare naive, selected AR, and selected ARMA on the same evaluation window.
    ...
    # Step 5: save the order table, metrics, backtests, and comparison figures.
    order_table_path = save_table_csv(
        order_table.set_index("order"),
        args.tables_dir,
        "week3_beginner_arma_order_table",
        repo_root=REPO_ROOT,
    )
    metrics_path = save_table_csv(
        metrics.set_index("model"),
        args.tables_dir,
        "week3_beginner_arma_metrics",
        repo_root=REPO_ROOT,
    )
    arma_backtest_path = save_table_csv(
        arma_backtest,
        args.tables_dir,
        "week3_beginner_selected_arma_backtest",
        repo_root=REPO_ROOT,
    )
    order_figure_paths = save_figure_pair(
        build_selection_figure(
            order_table,
            x_column="order",
            title="ARMA order ladder chosen by BIC",
        ),
        args.figures_dir,
        "week3_beginner_arma_order_selection",
        repo_root=REPO_ROOT,
    )
    backtest_figure_paths = save_figure_pair(
        build_backtest_comparison_figure(
            ...
            title="ARMA backtest: unemployment-rate changes",
        ),
        args.figures_dir,
        "week3_beginner_arma_backtest",
        repo_root=REPO_ROOT,
    )

    print("Week 3 beginner ARMA model")
    print()
